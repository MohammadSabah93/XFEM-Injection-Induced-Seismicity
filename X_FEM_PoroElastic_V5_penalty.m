...
% Normalize Displacement and Pressure Corrections**
        ...
        Sn(:,nt+1) = pn;
        Ss(:,nt+1) = pt;
        W(:,nt+1) = params.W0 + min(0, pn)*params.Dn_max./((params.Kn)*params.Dn_max + min(0, pn));
...
if current_time_hours >= 36
        fprintf('Reached total simulation time of 36 hours. Quitting simulation.\n');
        break;
end
end
toc;
...
save('simulation_Implicit_WTO_dt_1e-4.mat','-v7.3');
