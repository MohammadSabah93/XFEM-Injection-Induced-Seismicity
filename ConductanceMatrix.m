function globalH = ConductanceMatrix(params, connectivity, XYZ, PSI, PHI, NODES, CRACK, elementType)
...
mu=params.mu;
nCT = size(PHI,2);     
% Initialize the FE conductance matrix
[~,DOF_p,~] = calcDOF(NODES);
avgBW = 20;  % tune this to your mesh
nnzEst = avgBW * DOF_p;
globalH = spalloc(DOF_p,DOF_p,nnzEst);     
...
localH = 0;                                  % Initialize stiffness for current element
...
if (HEN == 0)
    ...
    localH = localH + gw(gpIndex) * dNGlobal' * (K/mu) * dNGlobal * det(jacobian);
...
for gpIndex = 1:length(gp)
    ...
    B=[Bstd,Benr];
    localH = localH + gw(gpIndex) * B' * (K/mu) * B * det(jacobian);

end
end
if length(localH) == 4                                                  % Unenriched element
        for row = 1:4
            for col = 1:4
                nIndexT = nIndexT+1;
                allRowT(nIndexT) = local(row);
                allColT(nIndexT) = local(col);
                allValT(nIndexT) = localH(row,col);
            end
        end
    else
        globalH(local,local) = globalH(local,local) + localH;            % Assemble the global stiffness
end

end

globalH = globalH + sparse(allRowT,allColT,allValT,DOF_p,DOF_p);
end
